#!/usr/bin/env python3
"""
Bijari 4K - Matriz 2x2 Video Sync
...
def combine_videos_4k():
    """Combina os 4 vídeos em uma matriz 2x2 para saída 4K."""
    print("Combinando vídeos em matriz 2x2 4K com FFmpeg...")
    ...
    # Comando principal: combinar 4 vídeos em matriz 2x2
    print("Combinando 4 vídeos em matriz 2x2 4K...")
    print(f"Layout: [Tela01] [Tela02]")
    print(f"        [Tela03] [Tela03]")
    ...
    main_cmd = [
        ffmpeg_path,
        "-i", INPUT_VIDEOS[0],  # Tela01
        "-i", INPUT_VIDEOS[1],  # Tela02
        "-i", INPUT_VIDEOS[2],  # Tela03
        "-i", INPUT_VIDEOS[3],  # Tela03 (repetido)
        "-filter_complex", filter_complex,
        "-map", "[v]",
        "-c:v", "libx264",
        "-preset", "ultrafast",
        "-crf", "23",  # Qualidade um pouco melhor para 4K
    ...
    if result.returncode == 0 and os.path.exists(OUTPUT_VIDEO) and os.path.getsize(OUTPUT_VIDEO) > 0:
        print(f"[OK] Vídeos combinados em matriz 2x2 4K com sucesso!")
        print(f"Arquivo gerado: {OUTPUT_VIDEO}")
        file_size = os.path.getsize(OUTPUT_VIDEO)
        print(f"Tamanho: {file_size / (1024*1024):.2f} MB")
        print(f"Resolução: {FINAL_WIDTH}x{FINAL_HEIGHT}")
        return True
    else:
        print(f"[ERRO] Falha ao combinar vídeos em matriz 2x2")
        if result.stderr:
...
def main():
    """Função principal."""
    print("=== Bijari 4K - Matriz 2x2 Video Sync ===")
    print(f"Layout: [Tela01] [Tela02]")
    print(f"        [Tela03] [Tela03]")
    print(f"Resolução: {FINAL_WIDTH}x{FINAL_HEIGHT}")
    print()
    ...
    if not os.path.exists(OUTPUT_VIDEO):
        print("Vídeo combinado 4K não encontrado. Criando...")
        if not combine_videos_4k():
            print("[ERRO] Falha ao criar vídeo combinado 4K!")
            return
        print(f"[OK] Vídeo combinado 4K criado: {OUTPUT_VIDEO}")
    else:
        print(f"[OK] Vídeo combinado 4K já existe: {OUTPUT_VIDEO}")
    ...
    print(f"VLC iniciado em {FINAL_WIDTH}x{FINAL_HEIGHT} (Borderless) e Loop!")
